...
function main() {
	...
	const usedDiskSpace = getDirectorySize(fileSystem)
	const updateSize = 30000000
	const diskSpaceToClear = updateSize - (totalDiskSpace - usedDiskSpace)

	const directorySizes = getDirectorySizes(fileSystem, [])
	const sufficientDirectories = Object.entries(directorySizes)
		.filter(([, size]) => size >= diskSpaceToClear)
		.sort((a, b) => a[1] - b[1])
	console.log(sufficientDirectories[0][1])
}
